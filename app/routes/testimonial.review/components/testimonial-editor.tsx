...
const TestimonialEditor: React.FC<TestimonialEditorProps> = ({
  ...
  return (
    <div
      className={cn(
        "flex flex-col items-center min-h-screen relative overflow-hidden",
        className,
      )}
    >
      <motion.div
      ...
      <Container
        className="h-full sm:px-32 gap-6 grow justify-center"
        variant="narrow"
      >
        <motion.div
        ...
        >
          ...
          >
                    ...
                    <div className="flex flex-col justify-center md:flex-row w-full h-full gap-2 p-6">
                      <div className="h-full aspect-square hidden md:block">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          >
                            <IconUploadPhoto className="mr-2 -ml-2" />
                            Upload Photo
                          </Button>
                          <Button
            ...
            <CarouselPrevious
              ref={previousRef}
              className={cn((slide === 0 || slide === 2) && "hidden")}
              variant="ghost"
            />
            <CarouselNext
              ref={nextRef}
              className={cn((slide === 1 || slide === 2) && "hidden")}
              variant="ghost"
            />
          </Carousel>
        </motion.div>
      ...
      >
        {Array.from({ length: 40 }).map((_value, index) => {
          const offset = index * 250 - 20 * 250;

          return (
            <ClientOnly key={`wave-${offset}`}>
              {() => (
                <LazyLottie
